...
def RLS(hi, x, s, M, lambdarls, delta):
    """RLS algorithm"""
    P=delta*np.eye(M)
    w=np.zeros(M)
    g=np.zeros(M)
...
def test_DCBF():
    ...
    for k in range(np.size(M)):
        ...
        for i in range(L):
            ...
            fDCBF_RLSDCD(hi, x, s, M[k], lambdarls, delta, 1)
            time1=time.perf_counter()
            TfRLSDCD[k,i]=time1-time0
            time0=time.perf_counter()
            fDCBF_RLSDCD(hi, x, s, M[k], lambdarls, delta, 4)
            time1=time.perf_counter()
            TfRLSDCD4[k,i]=time1-time0
            time0=time.perf_counter()
            RLS(hi, x, s, M[k], lambdarls, delta)
            time1=time.perf_counter()
            ...
            time0=time.perf_counter()
            RLSDCD(hi, x, s, M[k], lambdarls, delta, 1)
            time1=time.perf_counter()

            TRLSDCD[k,i]=time1-time0
            time0=time.perf_counter()
            RLSDCD(hi, x, s, M[k], lambdarls, delta, 4)
            time1=time.perf_counter()
    ...
    ResPython.to_csv("ResultsPythonMarlin.csv")
...
def test_RLS(N, M):
    ...
    s = signal.lfilter(hi, 1, x) + sigmav*rng.normal(size=N)
    (w, e) = RLS(hi, xx, s, M, lambdarls, delta)
    #plt.plot(np.log10(e**2))
    #plt.show()
...
def test_RLSDCD(N, M):
    ...
    xx = np.concatenate((np.zeros(M-1), x))
    s = signal.lfilter(hi, 1, x) + sigmav*rng.normal(size=N)
    (w, e) = RLSDCD(hi, xx, s, M, lambdarls, delta, Nu)
    #plt.plot(np.log10(e**2))
    #plt.show()
    print("MSD = ", np.linalg.norm(w-hi))
    return (hi, w)
...
def test_fRLSDCD(N, M):
    ...
    xx = np.concatenate((np.zeros(M-1), x))
    s = signal.lfilter(hi, 1, x) + sigmav*rng.normal(size=N)
    (w, e) = fDCBF_RLSDCD(hi, xx, s, M, lambdarls, delta, Nu)
    #plt.plot(np.log10(e**2))
    #plt.show()
    print("MSD = ", np.linalg.norm(w-hi))
    return (hi, w)
